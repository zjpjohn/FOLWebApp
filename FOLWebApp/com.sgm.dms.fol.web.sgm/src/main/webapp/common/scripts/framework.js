	...
	provider.$get = function($rootScope, $resource, $state, $location, $q) {
		...
		service.init = function() {
			...
			permissionDataResource.get(function(data) {
				...
				console.log("user name:"+$rootScope.user.name);
			}, function(err) {
				console.log(err);
			});
...
frameworkModule.controller('identityNumCtrl', [
	...
	/*
	 * $http({ method: 'GET', url: 'common/login/identity/num', headers:
	 * {'Content-Type': 'application/json' }
	 * }).success(function(data,status,headers,config){ $scope.num= data;
	...
	msgIdentityService.getSystemmounts($scope.data, 'system/systemamount/find',
			function(data, response_info) {
				$.messager.progress('close');
				$scope.num = data.rows.length;
			}, function(data,response_info) {
				$.messager.alert('提示','系统超时，请刷新');
				$.messager.progress('close');
				console.log(data);
			});
}]);